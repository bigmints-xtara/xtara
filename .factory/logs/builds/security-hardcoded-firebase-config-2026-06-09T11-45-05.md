# Build: security-hardcoded-firebase-config

**Date**: 2026-06-09T11:45:05.209Z
**Story**: /home/bigmints/projects/xtara/xtara-web/.factory/stories/features/security-hardcoded-firebase-config.yaml
**CLI**: pi
**Files**: 367

## Summary
Migrated hard-coded Firebase config to environment variables. Changes: (1) src/lib/firebase/firebase.ts now uses process.env.NEXT_PUBLIC_FIREBASE_* with dev fallbacks and production validation that throws on missing required vars. (2) src/app/layout.tsx uses process.env.NEXT_PUBLIC_GA_ID for Google Analytics GTag. (3) .env.example documents all required environment variables. (4) next.config.ts exposes all NEXT_PUBLIC_* vars to the client bundle. No Firebase credentials remain hard-coded in the client bundle.
