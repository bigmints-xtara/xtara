...
class CorrectedCrawler:
    ...
    def extract_courses_from_fees_page(self, soup, base_url):
        ...
        # First, try to extract from embedded JSON data (most accurate)
        json_courses = self.extract_courses_from_json_data(soup)
        if json_courses:
            courses.extend(json_courses)
        
        # Then try to extract from course cards (fallback)
    ...
    def extract_courses_from_url(self, url):
        ...
        try:
            ...
            # Try JSON data extraction first (most accurate)
            courses = self.extract_courses_from_json_data(soup)
            
            # If no courses found, try HTML parsing
            if not courses:
